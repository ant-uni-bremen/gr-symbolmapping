...
template <class T>
typename interleaver<T>::sptr interleaver<T>::make(
 ...
 */
template <class T>
interleaver_impl<T>::interleaver_impl(std::vector<size_t> interleaver_indices,
                                      bool is_packed,
      ...
      d_interleaver(new BitInterleaver(interleaver_indices))
{
    ...
    gr::sync_block::set_output_multiple(is_packed
                                            ? (d_interleaver->interleaverLength() / 8)
                                            : d_interleaver->interleaverLength());
}
...
template <class T>
void interleaver_impl<T>::interleave_packed(T* out,
                                            const T* in,
...
template <>
void interleaver_impl<uint8_t>::interleave_packed(uint8_t* out,
                                                  const uint8_t* in,
                                                  const unsigned nbytes_per_frame)
{
...
template <class T>
void interleaver_impl<T>::deinterleave_packed(T* out,
                                              const T* in,
                                              const unsigned nbytes_per_frame)
{
    throw std::logic_error(
...
template <>
void interleaver_impl<uint8_t>::deinterleave_packed(uint8_t* out,
                                                    const uint8_t* in,
                                                    const unsigned nbytes_per_frame)
{
    d_unpacker->unpack(d_unpacked_original.data(), in, nbytes_per_frame);
...
template <class T>
int interleaver_impl<T>::work(int noutput_items,
                              ...
                              gr_vector_void_star& output_items)
{
    ...
    const unsigned nframes =
        (d_is_packed ? 8 : 1) * noutput_items / d_interleaver->interleaverLength();
    const unsigned nbytes_per_frame =
        d_interleaver->interleaverLength() / (d_is_packed ? 8 : 1);

...
public:
    ...
    void map_to_constellation(fcmplx* symbols, const unsigned char* bits, const unsigned num_bits);
    void calculate_ln_probabilities(float* ln_probs, const fcmplx* rx_symbols,
                                    const unsigned num_symbols, const float snr_db);
    void calculate_ln_probabilities_vec(float* ln_probs, const fcmplx* rx_symbols,
    ...
    void demap_llrs(float * llrs, const fcmplx* rx_symbols,
                    const unsigned num_symbols, const float snr_db);
    void demap_llrs_vec(float * llrs, const fcmplx* rx_symbols,
                        const float* snr_lin, const unsigned num_symbols);
    ...
    void calculate_llrs_apriori(float* llrs, const float* apllrs, const float* ln_probs, const unsigned num_symbols);
...
private:
    ...
    void calculate_symbol_ln_probabilities(float* ln_probs, const fcmplx rx_symbol, const float scaling_factor);
    void calculate_symbol_llrs(float* llrs, const float *ln_probs);
    void calculate_symbol_llrs_apriori(float* llrs, const float* apllrs, const float *ln_probs);
    float calculate_apriori_sum(const float* apllrs, const unsigned bitmask);

    void demap_llrs_generic(float * llrs, const fcmplx* rx_symbols, const unsigned num_symbols, const float snr_db);
    void demap_llrs_bpsk(float * llrs, const fcmplx* rx_symbols, const unsigned num_symbols, const float snr_db);
    void demap_llrs_qpsk(float * llrs, const fcmplx* rx_symbols, const unsigned num_symbols, const float snr_db);
    void demap_llrs_8psk(float * llrs, const fcmplx* rx_symbols, const unsigned num_symbols, const float snr_db);
    void demap_llrs_16qam(float * llrs, const fcmplx* rx_symbols, const unsigned num_symbols, const float snr_db);

    void demap_llrs_vec_generic(float * llrs, const fcmplx* rx_symbols,
                                const float* snr_lin, const unsigned num_symbols);
    void demap_llrs_vec_bpsk(float * llrs, const fcmplx* rx_symbols,
                             const float* snr_lin, const unsigned num_symbols);
    void demap_llrs_vec_qpsk(float * llrs, const fcmplx* rx_symbols,
                             const float* snr_lin, const unsigned num_symbols);
    void demap_llrs_vec_16qam(float * llrs, const fcmplx* rx_symbols,
                              const float* snr_lin, const unsigned num_symbols);


    template<int constellation_size>
    void calculate_symbol_ln_probabilities_t(float* ln_probs, const fcmplx rx_symbol, const float scaling_factor){
    ...
    template<int constellation_size>
    void calculate_ln_probabilities_t(float* ln_probs, const fcmplx* rx_symbols, const unsigned num_symbols, const float snr_lin){
        const float scaling_factor = -0.5f * snr_lin;
        for (unsigned i = 0; i < num_symbols; i++) {
            calculate_symbol_ln_probabilities_t<constellation_size>(ln_probs, rx_symbols[i], scaling_factor);
            ln_probs += constellation_size;
        }
    }

    template<int constellation_size>
    void calculate_ln_probabilities_vec_t(float* ln_probs, const fcmplx* rx_symbols,
                                      const float* snr_lin, const unsigned num_symbols){
        for (unsigned i = 0; i < num_symbols; ++i) {
            const float scaling_factor = -0.5f * *snr_lin++;
            calculate_symbol_ln_probabilities_t<constellation_size>(ln_probs, rx_symbols[i], scaling_factor);
